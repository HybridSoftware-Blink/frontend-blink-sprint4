...
const passwordStrength = computed(() => {
  ...
  if (len < 6) return { text: 'Massa curta', color: 'text-red-600' };
  if (len < 10) return { text: 'Contrasenya acceptable', color: 'text-yellow-600' };
  return { text: 'Contrasenya forta', color: 'text-green-600' };
});
...
const handleSignup = () => {
  ...
  // Validació nom
  if (!form.name) {
    errors.name = 'Aquest camp és obligatori';
    return;
  }
  ...
  // Validació acceptar termes
  if (!form.acceptTerms) {
    errors.acceptTerms = 'Has d\'acceptar els termes i condicions';
    return;
  }
  
  console.log('Signup attempt with:', form);
  
  // Emit signup success per navegar al dashboard
  emit('signup-success');
};
</script>
          ...
          <div>
            <div class="flex items-start">
              ...
              <label for="acceptTerms" class="ml-2 block text-sm text-gray-700 cursor-pointer select-none">
                Accepto els 
                <a href="#" style="color: #89C242;" class="hover:brightness-75 transition-all">termes i condicions</a>
                i la
                <a href="#" style="color: #89C242;" class="hover:brightness-75 transition-all">política de privadesa</a>
              </label>
            </div>
